...
def prepare_data_for_training(X_train, y_train, X_val, y_val, X_test, y_test, vocab_size):
    """
    Prepare data with proper masking for padding tokens.
    Instead of padding targets to fixed length, create sequence length masks.
    """
    ...
    def pad_sequences_with_length(y, target_length, pad_idx=2):
        """Pad sequences with actual PAD token (index 2) and return actual lengths."""
        padded = np.full((y.shape[0], target_length), pad_idx, dtype=y.dtype)
        actual_lengths = np.zeros(y.shape[0], dtype=np.int32)
        ...
        return padded, actual_lengths

    y_train, y_train_lengths = pad_sequences_with_length(y_train, target_seq_len, pad_idx=2)
    y_val, y_val_lengths = pad_sequences_with_length(y_val, target_seq_len, pad_idx=2)
    y_test, y_test_lengths = pad_sequences_with_length(y_test, target_seq_len, pad_idx=2)
    ...
    logger.info(f"Sequence lengths: train min={y_train_lengths.min()} max={y_train_lengths.max()}")

    return (X_train, y_train_oh, y_train_lengths), (X_val, y_val_oh, y_val_lengths), (X_test, y_test_oh, y_test_lengths)


class MaskedCategoricalCrossentropy(keras.losses.Loss):
    """Categorical crossentropy that masks padding tokens."""

    def __init__(self, label_smoothing=0.1, pad_token_id=2, **kwargs):
        super().__init__(**kwargs)
        self.label_smoothing = label_smoothing
        self.pad_token_id = pad_token_id  # Index of '<PAD>' token
    ...
    def call(self, y_true, y_pred):
        ...
        y_true: (batch, time, vocab) one-hot encoded
        y_pred: (batch, time, vocab) predictions

        Padding tokens (argmax(y_true) == pad_token_id) are masked out.
        """
        # Compute loss for all positions
        loss = self.base_loss(y_true, y_pred)  # (batch, time)

        # Create mask for non-padding positions
        # Padding token index is 2 ('<PAD>')
        # Padding is when the one-hot target has max at class 2
        token_ids = tf.argmax(y_true, axis=-1)  # (batch, time)
        padding_mask = tf.equal(token_ids, self.pad_token_id)  # (batch, time)
        padding_mask = tf.cast(padding_mask, tf.float32)

        # Invert to get non-padding mask (we want to KEEP non-padding)
        non_padding_mask = 1.0 - padding_mask

        # Apply mask to loss: zero out loss at padding positions
        masked_loss = loss * non_padding_mask  # (batch, time)

        # Average only over non-padding positions
        sum_loss = tf.reduce_sum(masked_loss, axis=1)  # (batch,)
        count_non_padding = tf.reduce_sum(non_padding_mask, axis=1)  # (batch,)
        count_non_padding = tf.maximum(count_non_padding, 1.0)  # Avoid division by zero
...
def train_with_masking(language='kinyarwanda', epochs=100, batch_size=16):
    """Train model with proper masking for padding tokens."""
    logger.info("="*60)
    logger.info("LSTM Speech Recognition - Training with Masking")
    ...
    (X_train, y_train, y_train_lengths), (X_val, y_val, y_val_lengths), (X_test, y_test, y_test_lengths) = prepare_data_for_training(
    ...
    model = LSTMSpeechRecognition(
        ...
        lstm_units=512,
        dropout_rate=0.4,
        learning_rate=0.001  # Higher initial learning rate since we have masking now
    )
    model.build_bidirectional()
    model.get_summary()

    # Compile with masked loss function
    ...
    logger.info("Model compiled with masked loss function")

    # Create checkpoint directory
    checkpoint_dir = f'models/checkpoints/{language}_masked_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    logger.info(f"Loss function: MaskedCategoricalCrossentropy (ignores padding tokens)")
    ...
    # Save model
    final_model_path = f'models/trained/{language}_masked_final.h5'
...
if __name__ == "__main__":
    ...
    model, history, (loss, acc) = train_with_masking(
        language=language,
        epochs=epochs,
        batch_size=batch_size
    )
